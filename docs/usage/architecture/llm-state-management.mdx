...
This document dives deeply into how OpenHands preserves state throughout
an agent's lifecycle. It explains the `State` dataclass, how events are
recorded in history, how the controller updates metrics, and how sessions
can be persisted and restored. All of the relevant code is included so
presenters can reference exact line numbers during a seminar.

We begin with an overview of the key structures and then walk through each
component in detail.

## Complete `State` Dataclass

Below is the full implementation of `State` from
...
The dataclass captures all runtime information: the agent's iteration counters,
history of `Event` objects, current `AgentState`, and metrics. Sessions can be
serialized via `save_to_session` and later restored using
`restore_from_session`. During pickling the history is removed to reduce file
size; events are reloaded separately when the agent starts again.

## Controller Integration

The `AgentController` updates the state before and after each step. Key methods
live in `openhands/controller/agent_controller.py`. Below is the relevant
portion including `update_state_before_step` and `update_state_after_step`:
```python
    ...
    async def _react_to_exception(
        ...
        e: Exception,
```
The controller then calls `_step` which generates the messages for the next LLM
call and eventually publishes resulting actions to the `EventStream`. Relevant
details of how events are stored and the state updated are shown below. We
include a longer excerpt of `AgentController` so presenters can walk through the
logic line by line.
```python
...
When an agent finishes or the system shuts down, its state can be stored to a
session file. The main loop calls `save_to_session` as shown:
```python
...
```
The controller uses the `FileStore` interface to write the pickled and base64
encoded state. On the next startup `setup_agent` attempts to restore this
session:
```python
        logger.debug(
...
```
Restoring sets `agent_state` to `LOADING` so that `run_agent_until_done` starts a
fresh control loop. Once the event stream replays prior events the view cache is
rebuilt and the agent resumes processing.

## Full `AgentController` Source

For completeness the remainder of `AgentController` is included below. This
section is long but can be helpful if you want to step through the entire logic
...
The above source shows how the controller interacts with the `EventStream`,
updates history, increments iterations and merges cost metrics. By following the
code you can trace exactly how state is mutated across the lifetime of a
session.

## Conclusion

Maintaining state across LLM calls is central to OpenHands. The combination of
the `State` dataclass, the controller loop and session persistence ensures that
conversations can be paused, resumed or delegated without losing context.
